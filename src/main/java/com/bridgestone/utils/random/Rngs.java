package com.bridgestone.utils.random;/* -------------------------------------------------------------------------
 ...
 *
 * Name            : lib.Rngs.java  (Random Number Generation - Multiple Streams)
...
public class Rngs {
  ...
  // Consistent with the changes to the lib.Rvgs constructor, the seed[] array and
  // its associated variables should not be declared static.  If they are, 
  // different instances of the lib.Rngs class (which may be considered as
  // _different_ generators) would share the same seed[] array.  Instead,
  // each lib.Rngs instance should have its own copy of seed[].
 ...
 */
  public void testRandom() {
    ...
    if (ok)
      System.out.println("\n The implementation of lib.Rngs.java is correct");
    else
      System.out.println("\n ERROR - the implementation of lib.Rngs.java is not correct");
  }
}
